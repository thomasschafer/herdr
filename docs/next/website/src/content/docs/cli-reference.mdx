...
herdr workspace refresh-identity <workspace_id>
...
`workspace refresh-identity` re-pins a workspace's auto-derived name and git status to the
live directory of its first tab's root pane, clearing any manual custom name. It matters
when `[workspace] dynamic_naming` is `false` (see [Configuration](/docs/configuration/#workspace-naming)),
since the name and git status otherwise stay pinned to wherever the workspace last had its
identity set.
